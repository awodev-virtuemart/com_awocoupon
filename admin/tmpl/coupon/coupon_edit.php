...
//JFactory::getDocument()->addStyleSheet(com_awocoupon_ASSETS.'/css/jquery-ui.css');
//JFactory::getDocument()->addScript(com_awocoupon_ASSETS.'/js/jquery-ui.min.js');
//JFactory::getDocument()->addScript(com_awocoupon_ASSETS.'/js/jquery.ui.autocomplete.ext.js');
JFactory::getDocument()->addStyleSheet( '//cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.css' );
JFactory::getDocument()->addScript( '//cdnjs.cloudflare.com/ajax/libs/jqueryui/1.12.1/jquery-ui.js' );


?>

<style>.btn.btn-border { border: 1px solid #ccc; } </style>
...
<form action="index.php" method="post" name="adminForm" id="adminForm" class="form-validate">
	...
	<div class="width-50 fltlft">
		...
		<table class="admintable" style="width:100%;">
			<tr>
				...
				<td><input class="form-control w-100" type="text" name="coupon_code" maxlength="255" value="<?php echo ( ! empty( $this->item->coupon_code ) ? $this->item->coupon_code : '' ); ?>" /></td>
			</tr>
			<tr>
				<td class="key" nowrap><label><?php echo JText::_( 'COM_AWOCOUPON_CP_PUBLISHED' ); ?></label></td>
				<td><select name="published" class="form-select">
					<?php foreach ( $this->def_lists['published'] as $k => $v ) { ?>
						<option value="<?php echo $k; ?>" <?php if ( ! empty( $this->item->published ) && $this->item->published == $k ) echo 'SELECTED'; ?>><?php echo $v; ?></option>
					<?php } ?>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td><select name="coupon_value_type" class="form-select">
					<?php foreach ( $this->def_lists['coupon_value_type'] as $k => $v ) { ?>
						<option value="<?php echo $k; ?>" <?php if ( ! empty( $this->item->coupon_value_type ) && $this->item->coupon_value_type == $k ) echo 'SELECTED'; ?>><?php echo $v; ?></option>
					<?php } ?>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td><select name="discount_type" class="form-select">
					<?php foreach ( $this->def_lists['discount_type'] as $k => $v ) { ?>
						<option value="<?php echo $k; ?>" <?php if ( ! empty( $this->item->discount_type ) && $this->item->discount_type == $k ) echo 'SELECTED'; ?>><?php echo $v; ?></option>
					<?php } ?>
					</select>
				</td>
			</tr>
			<tr>
				...
				<td><input class="form-control w-100" type="text" name="coupon_value" maxlength="255" value="<?php echo ( ! empty( $this->item->coupon_value ) ? $this->item->coupon_value : '' ); ?>" /></td>
			</tr>
			...
			<tr><td class="key" nowrap><label><?php echo JText::_( 'COM_AWOCOUPON_CP_NUMBER_USES' ); ?></label></td>
				<td>
					<div class="row">
						<div class="col">
					<select name="num_of_uses_type" class="form-select">
					<?php foreach ( $this->def_lists['num_of_uses_type'] as $k => $v ) { ?>
						<option value="<?php echo $k; ?>" <?php if ( ! empty( $this->item->num_of_uses_type ) && $this->item->num_of_uses_type == $k ) echo 'SELECTED'; ?>><?php echo $v; ?></option>
					<?php } ?>
					</select>
						</div>
						<div class="col-auto">
					# <input class="form-control" type="text" name="num_of_uses" size="7" maxlength="255" value="<?php echo ( ! empty( $this->item->num_of_uses ) ? $this->item->num_of_uses : '' ); ?>" />
						</div>
					</div>
				</td>
			</tr>
			<tr><td class="key" nowrap><label><?php echo JText::_( 'COM_AWOCOUPON_CP_VALUE_MIN' ); ?></label></td>
				<td><input class="form-control w-100" type="text" name="min_value" maxlength="255" value="<?php echo ( ! empty( $this->item->min_value ) ? $this->item->min_value : '' ); ?>" /></td>
			</tr>
			<tr><td class="key"><label><?php echo JText::_( 'COM_AWOCOUPON_CP_DATE_START' ); ?></label></td>
				<td><?php echo JHTML::calendar( ! empty( $this->item->startdate ) ? substr( $this->item->startdate, 0, 10 ) : '', 'startdate', 'startdate', '%Y-%m-%d', [ 'size'=>'12', 'maxlength'=>'10', 'placeholder' => 'YYYY-MM-DD' ] );?></td>
			</tr>
			<tr><td class="key"><label><?php echo JText::_( 'COM_AWOCOUPON_CP_EXPIRATION' ); ?></label></td>
				<td><?php echo JHTML::calendar( ! empty( $this->item->expiration ) ? substr( $this->item->expiration, 0, 10 ) : '', 'expiration', 'expiration', '%Y-%m-%d', [ 'size'=>'12', 'maxlength'=>'10', 'placeholder' => 'YYYY-MM-DD' ] ); ?></td>
			</tr>
		</table>
	...
	<div class="width-50 fltrt">
		...
		<fieldset class="adminform">
		<legend><?php echo JText::_('COM_AWOCOUPON_CP_CUSTOMERS'); ?></legend>

			<div id="div_users" style="padding:10px;">
				<div id="div_user_simple_table">
					<div class="row">
						<div class="col pe-0">
							<input class="form-control" type="text" id="user_search" name="user_name" size="60" maxlength="255" value="" placeholder="<?php echo JText::_('COM_AWOCOUPON_GBL_SEARCH'); ?>" />
							<input type="hidden" name="user_id" value="" />
						</div>
						<div class="col-auto ps-0">
							<button type="button" class="btn btn-border" onclick="dd_itemselectf('user'); return false;"><?php echo JText::_('COM_AWOCOUPON_GBL_ADD'); ?></button>
				...
				<div id="div_user_advanced_table" style="display:none;">
					<div class="row">
						<div class="col pe-0">
							<input class="form-control" type="text" id="user_search_txt" size="60" onkeyup="dd_searchg('user')" placeholder="<?php echo JText::_('COM_AWOCOUPON_GBL_SEARCH'); ?>">
						</div>
						<div class="col-auto ps-0">
							<button type="button" class="btn btn-border" onclick="dd_itemselectg('user'); return false;"><?php echo JText::_('COM_AWOCOUPON_GBL_ADD'); ?></button>
				...
				<div class="function_type2_holder">
					...
					<tbody>
					<?php  if ( ! empty( $this->item->userlist ) && is_array( $this->item->userlist ) ) { foreach ( $this->item->userlist as $row ) { ?>
						<tr id="tbl_users_tr<?php echo $row->user_id; ?>">
							<td><?php echo $row->user_id; ?></td>
							<td><?php echo $row->user_name; ?></td>
							<td class="last pe-0" align="right">
								<button type="button" class="btn btn-close" onclick="deleterow('tbl_users_tr<?php echo $row->user_id; ?>');return false;" ></button>
		...
		<fieldset class="adminform">
		<legend><?php echo JText::_('COM_AWOCOUPON_CP_PRODUCTS'); ?></legend>
			
			<div style="padding:10px;">
				<div id="div_asset1_type">
					<span id="span_asset1_type">
						<select name="asset1_function_type" class="form-select" onchange="asset_type_change();">
							<option value="">- <?php echo JText::_('COM_AWOCOUPON_GBL_TYPE'); ?> -</option>
							<option value="product" <?php if ( ! empty( $this->item->asset1_function_type ) && $this->item->asset1_function_type == 'product' ) echo 'SELECTED'; ?>><?php echo JText::_( 'COM_AWOCOUPON_CP_PRODUCT' ); ?></option>
							<option value="category" <?php if ( ! empty( $this->item->asset1_function_type ) && $this->item->asset1_function_type == 'category' ) echo 'SELECTED'; ?>><?php echo JText::_( 'COM_AWOCOUPON_CP_CATEGORY' ); ?></option>
						</select>
					</span>
				</div>
				...
				<div id="div_asset1_inner" >
					<div id="div_asset_simple_table">
						<div class="row">
							<div class="col pe-0">
								<input class="form-control" type="text" id="asset_search" name="asset_name" size="60" maxlength="255" value="" placeholder="<?php echo JText::_('COM_AWOCOUPON_GBL_SEARCH'); ?>" />
								<input type="hidden" name="asset_id" value="" />
							</div>
							<div class="col-auto ps-0">
								<button type="button" class="btn btn-border" onclick="dd_itemselectf('asset'); return false;"><?php echo JText::_('COM_AWOCOUPON_GBL_ADD'); ?></button>
					...
					<div id="div_asset_advanced_table" style="display:none;">
						<div class="row">
							<div class="col pe-0">
								<input class="form-control" type="text" id="asset_search_txt" size="60" onkeyup="dd_searchg('asset')" placeholder="<?php echo JText::_('COM_AWOCOUPON_GBL_SEARCH'); ?>">
							</div>
							<div class="col-auto ps-0">
								<button type="button" class="btn btn-border" onclick="dd_itemselectg('asset'); return false;"><?php echo JText::_('COM_AWOCOUPON_GBL_ADD'); ?></button>
					...
					<div class="function_type2_holder">
						...
						<thead><tr><th><?php echo JText::_('COM_AWOCOUPON_GBL_ID'); ?></th><th><?php echo JText::_('COM_AWOCOUPON_GBL_NAME'); ?></th><th>&nbsp;</th></tr></thead>
						<tbody>
						<?php  if ( ! empty( $this->item->assetlist ) && is_array( $this->item->assetlist ) ) { foreach( $this->item->assetlist as $row ) { ?>
							<tr id="tbl_assets_tr<?php echo $row->asset_id; ?>">
								<td><?php echo $row->asset_id; ?></td>
								<td><?php echo $row->asset_name; ?></td>
								<td class="last pe-0" align="right">
									<button type="button" class="btn btn-close" onclick="deleterow('tbl_assets_tr<?php echo $row->asset_id; ?>');return false;" ></button>
	...
	<input type="hidden" name="id" value="<?php echo ( ! empty( $this->item->id ) ? $this->item->id : '' ); ?>" />
	...
	jQuery( "#user_search" )
		.autocomplete({
			source: function( request, response ) {
				jQuery.getJSON( 
					base_url, 
					{option:my_option, task:'users_x', tmpl:'component', no_html:1,term: request.term}, 
					response 
				);
			},
			minLength: 2,
			...
			select: function( event, ui ) {
				if ( ui.item ) {
					...
					document.adminForm.user_id.value = ui.item.id;
					dd_itemselectf( 'user' );
				}
			}
		})
...
function submitbutton(pressbutton) {
	...
	if(err != '') alert(err);
		...
		if(trim(form.expiration.value)!='') {
			...
			d = d*1;
			if(d < <?php echo AwocouponHelper::instance()->getDate(null,'Ymd'); ?>) {
				if(!confirm(str_coup_err_confirm_expiration)) is_submit = false;
			}
		}
		if(is_submit) Joomla.submitform( pressbutton );
	}
...
function view_all(type) {
	...
	jQuery.getJSON( 
		base_url, 
		{option:my_option, task: field + 's_x', term:"___ALL___", tmpl:'component', no_html:1}, 
		function(data) {
	...
	if(trim(id)!='') {
		...
		jQuery('#'+tbl+' > tbody:last').append(
			'<tr id="'+tbl+'_tr'+id+'">'+
				...
				'<td>'+name+'</td>'+
				'<td class="last pe-0" align="right">'+
						'<button type="button" class="btn btn-close" onclick="deleterow(\''+tbl+'_tr'+id+'\');return false;" ></button>'+
		...
		jQuery( "#asset_search" )
			.autocomplete({
				source: function( request, response ) {
					jQuery.getJSON( 
						base_url, 
						{option:my_option, task: val + 's_x', tmpl:'component', no_html:1,term: request.term}, 
						response 
					);
				},
				minLength: 2,
				...
				select: function( event, ui ) {
					if ( ui.item ) {
						jQuery( this ).val( ui.item.label );
						document.adminForm.asset_id.value = ui.item.id;
						dd_itemselectf( 'asset' );
					}
				}
			})
